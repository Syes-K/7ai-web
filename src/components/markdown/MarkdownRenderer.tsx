...
import "./markdown-renderer-hljs.css";

export type MarkdownRendererVariant = "chat" | "console";
...
const VARIANT_ROOT_CLASS: Record<MarkdownRendererVariant, string> = {
  chat: "markdown-renderer markdown-renderer--chat min-w-0 w-full max-w-full break-words break-anywhere text-zinc-100",
  console:
    "markdown-renderer markdown-renderer--console min-w-0 w-full max-w-full break-words break-anywhere text-white/85",
};
...
export function MarkdownRenderer({ body, variant = "chat", className }: Props) {
  const rootClass = `${VARIANT_ROOT_CLASS[variant]} ${className ?? ""}`.trim();
  return (
